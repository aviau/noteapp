...
impl AppConfigService {
    pub fn init_from_app_handle(app_handle: &tauri::AppHandle) -> Result<Self, Box<dyn Error>> {
        let config_dir_path = match app_handle.path_resolver().app_config_dir() {
            Some(config_dir) => config_dir,
            None => return Err("could not find app config directory".into()),
        };
...
fn get_or_create_config(config_file_path: &PathBuf) -> Result<AppConfig, Box<dyn Error>> {
...
fn read_config_file(config_file_path: &PathBuf) -> Result<Option<AppConfig>, Box<dyn Error>> {
